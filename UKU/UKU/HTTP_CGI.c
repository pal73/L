...
U16 cgi_func (U8 *env, U8 *buf, U16 buflen, U32 *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'a' :
      /* параметры загружаемые при загрузке*/
      switch (env[2]) {
        case 'a':
          /* Местоположение */
          len = sprintf((char *)buf,"<location>%s</location>","Новосибирск" );
          break;
        case 'b':
          /* Модель*/
		len = sprintf((char *)buf,"<model>%s</model>","Новосибирск1" );;
          break;
        case 'c':
          /* Серийный номер*/
          len = sprintf((char *)buf,"<serial>%d</serial>",12345 );
          break;
        case 'p':
 ...
 //                   env[1],(get_button () & (1 << (env[1]-'0'))) ? "true" : "false");
    ...
    case 'l':
      /* Button state - xml file 'button.cgx' */
      len = sprintf((char *)buf,"<value>%d</value>",short_plazma );
      break;
  }
  return ((U16)len);
}
